...
public class MovieServiceTest {
    ...
    public void shouldUpdateMovie() throws Exception {
        Movie m = this.service.findByTitle("The Avengers");
        m.setTitle("Avengers:Civil War");

        Movie movieFromDB = this.service.update(m.getId(),m);

        assertNotNull(movieFromDB);
        assertEquals(1,movieFromDB.getId());
        assertEquals("Avengers:Civil War",movieFromDB.getTitle());
